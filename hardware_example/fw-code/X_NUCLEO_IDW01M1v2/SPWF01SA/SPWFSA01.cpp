...
bool SPWFSA01::connect(const char *ap, const char *passPhrase, int securityMode)
{
    ...
    //pc3.printf("\r\nSENDING: [AT+S.SCFG=wifi_wpa_psk_text,%s]", passPhrase);
    //if(!(_parser.send("AT+S.WIFI=0") && _parser.recv("OK"))) 
     //   {   
      //      return false;
       // } 
    ...
    //AT+S.SSIDTXT=%s\r
    //pc3.printf("\r\nSENDING: [AT+S.SSIDTXT=%s]", ap);
    if(!(_parser.send("\r\nAT+S.SSIDTXT=%s", ap) && _parser.recv("OK"))) 
        {   
            pc3.printf("\r\nSPWF> error ssid set\r\n");
    ...
    //pc3.printf("\r\nSENDING: [AT+S.SCFG=wifi_priv_mode,%d]", securityMode);
    
    // ADDED FROM THE SLIDES 
    if(!(_parser.send("AT+S.SCFG=wifi_auth_type,0") && _parser.recv("OK"))) 
        {   
    ...
    /*set idle mode (0->idle, 1->STA,3->miniAP, 2->IBSS)*/
    //pc3.printf("\r\nSENDING: [AT+S.SCFG=wifi_mode,%d]", 1);
    ...
    // Added following the slides.
    //if(!(_parser.send("AT+S.WIFI=1") && _parser.recv("OK"))) 
     //   {   
     //       return false;
      //  } 
    //AT&W
    ...
    //pc3.printf("\r\nResetting module again");
    //reset module
    reset();
      
    //pc3.printf("\r\n+Waiting for [WIND:35:WiFi Scan Complete (0x0)]");
    //while(1)
     //   if((_parser.recv("+WIND:35:WiFi Scan Complete (0x0)")))
      //      {
       //         break;
        //    }   
            
    pc3.printf("\r\n+Waiting for [WIND:19:WiFi Join]");
    while(1)
        if((_parser.recv("+WIND:19:WiFi Join:2C:30:33:5C:99:94"))) // this is the MAC of the AP 
            {
                break;
            }   

    //pc3.printf("\r\n+Waiting for [WIND:25:WiFi Association]");
    while(1)
        if((_parser.recv("+WIND:25:WiFi Association with 'dd-wrt' successful")))
            {
                break;
            }  
  
    //pc3.printf("\r\n+Waiting for [+WIND:51:WPA Handshake Complete]");
    while(1)
        if((_parser.recv("+WIND:51:WPA Handshake Complete")))
            {
                break;
            }          

    pc3.printf("\r\nWaiting for WIND:24:WiFi Up...");
    while(1)
        if((_parser.recv("+WIND:24:WiFi Up:192.168.1.102"))) // This is the IP given from the router to the device
            {
                break;
            }    
